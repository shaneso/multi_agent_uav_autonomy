...
### Prerequisites (packages) to install when in ROS-Docker CLI
...
Next you run `catkin_make` to build the ROS environment. In your main project directory root, you will see a `build` folder, a `devel` folder, and a `src` folder (which contains all the folders containing relevant source code and software).
